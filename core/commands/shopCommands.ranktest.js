...
// 🏷️ RANK FILTER (2026-09-17, owner: "add a filter by rank ... .j shop equipment legendary"):
// `.j shop <category> <rank>` filters a category view by item rank/rarity,
// `.j shop <rank>` alone filters the whole shop. Rank words are only treated
// as a rank when the REST of the query is a known category (or empty), so
// plain name-search (`.j shop rare sword`, an item literally named "Rare ...")
// keeps working exactly as before.
const SHOP_RARITIES = ['COMMON', 'UNCOMMON', 'RARE', 'EPIC', 'LEGENDARY', 'MYTHIC'];

function resolveShopRarity(tok) {
    if (!tok) return null;
    const t = String(tok).toLowerCase();
    if (t.length < 3) return null;
    return SHOP_RARITIES.find((r) => r.toLowerCase().startsWith(t)) || null;
}

// "equipment legendary" -> { cat: 'equipment', rarity: 'LEGENDARY' }
// "legendary"          -> { cat: 'all',      rarity: 'LEGENDARY' }
// "rare potion"        -> { cat: 'rare potion', rarity: null }  (name search)
function parseShopQuery(raw) {
    const tokens = String(raw || 'all').trim().split(/\s+/).filter(Boolean);
    let rarity = null;
    const kept = [];
    for (const tok of tokens) {
        const r = rarity ? null : resolveShopRarity(tok);
        if (r) { rarity = r; continue; }
        kept.push(tok);
    }
    let cat = kept.join(' ') || 'all';
    // Only honour the rank word when what remains is a category view (or empty).
    if (rarity && kept.length > 0 && !KNOWN_CATEGORIES.has(cat.toLowerCase())) {
        rarity = null;
        cat = tokens.join(' ');
    }
    return { cat, rarity, original: tokens.join(' ') };
}
...
async function displayShop(sock, chatId, category = 'all') {
    ...
    // 🏷️ rank filter: `.j shop equipment legendary` / `.j shop legendary`
    const { cat, rarity, original } = parseShopQuery(category);
    const byRank = (item) => !rarity || String(item.rarity || 'COMMON').toUpperCase() === rarity;
    const rankTag = rarity ? ` • 🏷️ ${rarity}` : '';

    // 💡 DEDICATED SUMMON SHOP: if category is 'summon', show only summon items
    if (cat.toLowerCase() === 'summon') {
        const summonEntries = Object.entries(summonItems).filter(([, i]) => byRank(i));
        if (summonEntries.length === 0) {
            await sock.sendMessage(chatId, { text: rarity ? `🥚 No ${rarity.toLowerCase()} summon items available.` : '🥚 No summon items available.' });
            return;
        }
    ...
    if (!KNOWN_CATEGORIES.has(cat.toLowerCase())) {
        const q = cat.toLowerCase().trim();
        const qFlat = q.replace(/\s+/g, '_'); // "health potion" also matches id "health_potion"
        const fullPool = [...Object.values(classItems), ...Object.values(mainItems), ...Object.values(summonItems)];
        const pool = rarity ? fullPool.filter(byRank) : fullPool;
        const scored = [];
        ...
        let msg = `🔍 *SHOP SEARCH* - "${original}"${rarity ? ` • 🏷️ ${rarity}` : ''}\n`;
        msg += `━━━━━━━━━━━━━━━\n\n`;
        if (results.length === 0) {
            msg += `❌ Nothing matches "${original}".\n\n`;
            msg += `📂 Categories: \`${p} shop all · equipment · class · quest · permanent · summon\` · 🏷️ Rank: \`${p} shop equipment legendary\`\n`;
        } else {
    ...
    const activeCat = categoryInfo[cat.toLowerCase()] || categoryInfo.all;

    let msg = `${activeCat.icon} *SHOP*${cat.toLowerCase() !== 'all' ? ` • ${activeCat.name}` : ''}${rankTag}\n`;
    msg += `━━━━━━━━━━━━━━━\n`;
    msg += `📂 \`${p} shop all · equipment · class · quest · permanent\` · 🏷️ \`${p} shop equipment legendary\` · 🔍 \`${p} shop <name>\`\n\n`;

    // Filter items by category (+ optional rank)
    const filteredItems = Object.entries(items).filter(([key, item]) => {
        if (!byRank(item)) return false;
        if (cat.toLowerCase() === 'all') return true;
        return item.category.toLowerCase() === cat.toLowerCase();
    });

    if (filteredItems.length === 0) {
        msg += `❌ No ${rarity ? rarity.toLowerCase() + ' items in this' : 'items found in this'} category.\n`;
    } else {
