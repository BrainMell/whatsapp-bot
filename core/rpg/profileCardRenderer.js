...
// Class → full-body sprite variants (files resolved via loadChar: clean/ copy
// first, then the original asset dir).
// 🧩 SPRITE CONSISTENCY 2026-09-17: this map now MIRRORS the Go renderer's
// registry (bot_generation pkg/combat/sprites.go CharacterSprites) key-for-key
// and file-for-file, INCLUDING evolved/ascended classes. Variant selection is
// `list[spriteIndex % list.length]` - the exact same resolution adventures,
// duels and every themed Go card use. Previously this card drew variant 0 of a
// tiny base-class map (or apprentice1.png for evolved classes), so the sprite
// on the profile card did not match the player's assigned sprite elsewhere.
const CLASS_SPRITE_SETS = {
  FIGHTER: ['Fighter1.png', 'fighter2.png', 'fighter3.png'],
  SCOUT: ['scout1.png', 'scout2.png', 'scout3.png', 'sout4.png'],
  RANGER: ['scout1.png', 'scout2.png', 'scout3.png', 'sout4.png'],
  ...
  // Ascended classes - mapped to closest thematic sprites (same as Go)
...
// Back-compat alias (variant 0 per class)
const CLASS_SPRITES = Object.fromEntries(Object.entries(CLASS_SPRITE_SETS).map(([k, v]) => [k, v[0]]));
